...
#define RUSTDESK_LIB_PATH "libzoranderdesk.so"
...
bool flutter_rustdesk_core_main() {
   void* libzoranderdesk = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   if (!libzoranderdesk) {
      fprintf(stderr,"Failed to load \"libzoranderdesk.so\"\n");
      char* error;
   ...
   auto core_main = (RustDeskCoreMain) dlsym(libzoranderdesk,"rustdesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
